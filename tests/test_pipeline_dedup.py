...
def make_pred(hours_ago: float, estimated_prob: float) -> dict:
    ts = (datetime.now(timezone.utc) - timedelta(hours=hours_ago)).isoformat()
    return {"timestamp": ts, "estimated_prob": estimated_prob}
...
def test_skip_when_recent_and_price_stable():
    pred = make_pred(hours_ago=1.0, estimated_prob=0.65)
    assert should_skip(pred, market_price=0.66) is True  # 1h ago, moved 1pp


def test_no_skip_when_old():
    pred = make_pred(hours_ago=5.0, estimated_prob=0.65)
...
def test_no_skip_when_just_over_4h():
    pred = make_pred(hours_ago=4.01, estimated_prob=0.65)
